...
function CaToolbar(options){
	...
	if(!this.elt){
		error(`${this.name}:No Main Container...`);
		return;
	}
...
CaToolbar.prototype.__createDropDown = function(options){
	...
	if(options.callback){
		...
		lists.forEach( radio => radio.addEventListener('change',getStatus.bind(this) ));
	}
...
CaToolbar.prototype.__refresh = function(){
	...
	handler_icon.textContent = 'arrow_left';
...
/*
 * @private 
 * __collapse - collapse sub tools.
 */
CaToolbar.prototype.__collapse = function(){
	this.handler.querySelector('label').textContent = 'arrow_right';
	this._sub_tools.forEach(btn => btn.style.display = 'none');
	//this.btns.forEach(btn => btn.parentElement.style.width = 0);
}
/*
 * @private 
 * __expand - expand sub tools.
 */
CaToolbar.prototype.__expand = function(){
	this.handler.querySelector('label').textContent = 'arrow_left';
	this._sub_tools.forEach(btn => btn.style.display = '');
	//this.btns.forEach(btn => btn.parentElement.style.width = '2.4rem');
}
