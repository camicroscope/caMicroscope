...
LayersViewer.prototype.__search = function(e){
	...
	items.forEach(item=>{
		if(!item.name.match(regex))
			this.elt.querySelector(`#_${item.id}`).style.display = 'none';
	});
}
/*
...
LayersViewer.prototype.__covertMode = function(){
	if(!this.setting.data){
		console.warn(`${className}: No Raw Data`);
		return; 
	}

	this._v_model =  this.setting.data.reduce(function(model, item){
...
LayersViewer.prototype.__change = function(e){
	...
	if(dataset.type !== 'leaf') {
		if(!window.confirm(`Do you really want to show ${dataset.name}'s layers?`)) {
		...
		// call back function
		this.setting.callback.call(null,(id?this._v_model[id].data:this.setting.data),chkVal);
		//console.log(chkVal);
