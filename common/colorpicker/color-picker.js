...
(function(win, doc, NS) {
  ...
  (function($) {
    ...
    var on_down = 'touchstart mousedown';
    var on_move = 'touchmove mousemove';
    var on_up = 'touchend mouseup';
    var on_resize = 'orientationchange resize';
    ...
    function point(el, e) {
      ...
      var X = 'clientX';
      var Y = 'clientY';
      var x = !!e[T] ? e[T][0][X] : e[X];
      var y = !!e[T] ? e[T][0][Y] : e[Y];
      var o = offset(el);
      return {
        x: x - o.l,
        y: y - o.t,
    ...
    var HSV = get_data([0, 1, 1]); // default is red
    var H = c[0];
    var SV = c[1];
    var H_point = H[first];
    var SV_point = SV[first];
    var start_H = 0;
    var start_SV = 0;
    var drag_H = 0;
    var drag_SV = 0;
    ...
    function create(first, bucket) {
      ...
      P_H = size(picker).h;
      var SV_size = size(SV);
      var SV_point_size = size(SV_point);
      var H_H = size(H).h;
      var SV_W = SV_size.w;
      var SV_H = SV_size.h;
      var H_point_H = size(H_point).h;
      var SV_point_W = SV_point_size.w;
      var SV_point_H = SV_point_size.h;
      if (first) {
        picker.style.left = picker.style.top = '-9999px';
        function click(e) {
          var t = e.target;
          var is_target = t === target || closest(t, target) === target;
          if (is_target) {
            create();
          } else {
            $.exit();
      ...
      function do_SV(e) {
        var o = point(SV, e);
        var x = edge(o.x, 0, SV_W);
        var y = edge(o.y, 0, SV_H);
        HSV[1] = 1 - ((SV_W - x) / SV_W);
        HSV[2] = (SV_H - y) / SV_H;
      ...
      function stop(e) {
        ...
        var a = [HSV2HEX(HSV), $];
        var is_target = t === target || closest(t, target) === target;
        var is_picker = t === picker || closest(t, picker) === picker;
    ...
    $.fit = function(o) {
      var w = size(win);
      var y = size(h);
      var screen_w = w.w - y.w; // vertical scroll bar
      var screen_h = w.h - h.clientHeight; // horizontal scroll bar
      var ww = offset(win);
      ...
      if (is_object(o)) {
        ...
        var min_x = ww.l;
        var min_y = ww.t;
        var max_x = ww.l + w.w - P_W - screen_w;
        var max_y = ww.t + w.h - P_H - screen_h;
