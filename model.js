...
async function initUIcomponents() {
  ...
  // function to delete uploaded model
  ...
  (function (callback) {
    for (let key in data) {
      let name = key.split("/").pop(),
          ...
          //next line creates button 
          del=`<input type="submit" value="Remove" onclick="return confirm('Are you sure want remove this Model.');"/>`,
          row = table.insertRow(),
...
deleteButton.setAttribute('name', cursor.value.taskTitle);
...
function deleteItem(event) {
    ...
    let dataTask = event.target.getAttribute('deleterow');

    // open a database transaction and delete the task, finding it by the name we retrieved above
    let transaction = db.transaction(["models_store"], "readwrite");
    let request = transaction.objectStore("models_store").delete(dataTask);
