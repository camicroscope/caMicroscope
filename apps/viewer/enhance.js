...
function clahe(imgData, tiles=32, clip=0.1) {
  ...
  var invTileSize = [1.0 / tilesize[0], 1.0 / tilesize[1]];
  ...
  for (var i=0; i<ytiles; i++) {
    ...
    for (var j=0; j<xtiles; j++) {
      ...
      var hist = histogram(intens, x0, y0, x1, y1, numBins, h, w);

      cdfs[i][j] = buildcdf(hist, numBins, tilesize[0] * tilesize[1], clipLimit);
    }
  }
  ...
  for (var y=0, idx=0; y<h; ++y) {
    for (var x=0; x<w; ++x, idx+=4) {
      ...
      var tx = x * invTileSize[0] - 0.5;
      var ty = y * invTileSize[1] - 0.5;
...
function edgedetect(imgData, threshold, is, js, sx, sy) {
  ...
  for (var i=is; i<is+sx; i++) {
    for (var j=js; j<js+sy; j++) {
      var ox = getpix(imgData.data, j, i, imgData.width);
      var nx = getpix(imgx, j-js, i-is, sx);
      var ny = getpix(imgy, j-js, i-is, sx);
...
function applyfilter(imgData, kernel, is, js, sx, sy) {
  ...
  var sumkr=0; var sumkg=0; var sumkb=0;
  ...
  var x1=is-kernelSizex2; var x2=is+sx-kernelSizex2;
  var y1=js-kernelSizey2; var y2=js+sy-kernelSizey2;
...
function buildcdf(hist, numBins, numPixels, clipLimit) {
  var excess = 0;
