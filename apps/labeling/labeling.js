...
function getImage(result, callback) {
  ...
  // `${window.location.origin}/img/IIP/raw/?
  // IIIF=${data.location}/${size.x},${size.y},${size.width},${size.height}/${widthInClient},/0/default.jpg`
  // : `${data.location}/${size.x},
  const url =
    ImgloaderMode == 'iip' ?
...
function createLabelList() {
  ...
  $UI.modalbox.open();
  const continueBtn = $UI.modalbox.elt.querySelector(
      '.modalbox-footer button.continue',
  );
  const saveBtn = $UI.modalbox.elt.querySelector(
...
async function saves(ROI, subROIs) {
  ...
  try {
    ...
    // statements
    $UI.message.addError('Save Labels Failed');
    console.error(error);
  } finally {
    console.log('done');
  }
