  ...
  $UI.toolbar = new CaToolbar({
    ...
    subTools: [
        ...
        callback: function() {
          if (window.location.search.length > 0) {
            window.location.href =
              "../viewer/viewer.html" + window.location.search;
          } else {
      ...
      },
      {
        ...
        callback: () => {
          window
            .open("https://goo.gl/forms/mgyhx4ADH0UuEQJ53", "_blank")
            .focus();
        }
      }
    ]
...
function initCore() {
  ...
  $CAMIC.viewer.addOnceHandler("open", function(e) {
    ...
    $UI.modelPanel.__btn_save.addEventListener(
      "click",
      function(e) {
    ...
    $UI.modelPanel.__btn_savecsv.addEventListener(
      "click",
      function(e) {
...
function drawRectangle(e) {
  ...
  if (e.checked) {
    ...
    required_zoom = $UI.args
      ? parseInt($UI.args.status.split("_")[1].split("-")[1])
      : current_zoom;
    if (current_zoom != required_zoom) {
      alert(
        `You are testing the model for a different zoom level (recommended: ${required_zoom}). Performance might be affected.`
      );
    }
...
function checkSize(imgColl, imagingHelper) {
  ...
  const min = imagingHelper._viewer.viewport.imageToViewportCoordinates(
    top_left[0],
    top_left[1]
  );
  const max = imagingHelper._viewer.viewport.imageToViewportCoordinates(
    bottom_right[0],
    bottom_right[1]
  );
...
function runPredict(key) {
  ...
  const prefix_url =
    ImgloaderMode == "iip"
      ? `../../img/IIP/raw/?IIIF=${$D.params.data.location}`
      : $CAMIC.slideId;
  ...
  store.get(key).onsuccess = async function(e) {
    ...
    tf.tidy(() => {
      const warmupResult = model.predict(
        tf.zeros([1, image_size, image_size, input_channels])
      );
    ...
    for (let y = Y, dy = 0; y < Y + totalSize; y += step) {
      ...
      for (let x = X; x < X + totalSize; x += step) {
        ...
        let imgData = fullResCvs
          .getContext("2d")
          .getImageData(0, 0, fullResCvs.width, fullResCvs.height);
        tf.tidy(() => {
          const img = tf.browser.fromPixels(imgData).toFloat();
          let img2;
          if (input_channels == 1) {
            img2 = tf.image
              .resizeBilinear(img, [image_size, image_size])
              .mean(2);
          } else {
          ...
          if (scaleMethod == "norm") {
            ...
            let std = img2
              ...
              .sum()
              .div(img2.flatten().shape)
              .sqrt();
            normalized = img2.sub(mean).div(std);
          }
          let batched = normalized.reshape([
            1,
            image_size,
            image_size,
            input_channels
    ...
    self.showResults(
      "" + i + ": " + classes[i_max] + " - " + final[i_max].toFixed(3)
    );
    self.hideProgress();
    model.dispose();
  };
...
function uploadModel() {
  ...
  submit.addEventListener("click", async function(e) {
    ...
    if (
      ...
      _classes.value &&
      _image_size.value &&
      ((!toggle.checked && topology.files[0].name.split(".").pop() == "json") ||
        (toggle.checked && url))
    ) {
      ...
      if (toggle.checked) {
        ...
        var modelInput = tf.io.browserFiles([
          topology.files[0],
          ...weights.files
        ]);
      }
      ...
      try {
        ...
        try {
          ...
          status.innerHTML =
            "Model failed on the given values of patch size. Please input values on which the model was trained.";
          console.log(e);
        ...
        if (toggle.checked) status.innerHTML = "Please enter a valid URL.";
          ...
          status.innerHTML =
            "Please enter a valid model. Input model.json in first input and all weight binaries in second one without renaming.";
        console.error(e);
      }
    } else {
  ...
  (function(callback) {
    for (let key in data) {
      let name = key.split("/").pop(),
        ...
        size =
          (data[key].modelTopologyBytes +
            data[key].weightDataBytes +
            data[key].weightSpecsBytes) /
          (1024 * 1024),
        row = table.insertRow(),
        classes,
        input_shape,
      ...
      if (name.slice(0, 4) == "pred") {
        store.get(name).onsuccess = function(e) {
          ...
          td = row.insertCell();
          td.innerHTML =
            '<button class="btn btn-primary btn-xs my-xs-btn" id="removeModel" type="button">Remove Model</button>';
          document
...
function convertCoordinates(imagingHelper, bound) {
  ...
  for (let i = 0; i < newArray.length; i++) {
    ...
    for (let j = 0; j < boundElement.length; j++) {
      newArray[i][j] =
        j === 0
          ? imagingHelper.dataToPhysicalX(boundElement[j])
