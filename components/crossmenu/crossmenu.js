...
function AddBody(entry, display = '') {
  ...
  let button = `<td><button class=\"crossMenuBtn\" data-id='${entry.id}' data-name='${entry.name}' data-location='${entry.location}' onclick='OpenCrossViewer(this)'>
  <span class="material-icons">open_in_browser</span></button></td>`;
...
function CreateCheckbox(val) {
  ...
  <input name="filter-val" type="checkbox" class="form-check-input" onchange="HandleFilterChange(this)" name=${val} id=${val} value=${val} checked="true">
  <label for=${val} class="form-check-label">${val}</label></div>`);
}
