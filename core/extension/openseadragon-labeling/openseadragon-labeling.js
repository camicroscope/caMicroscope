...
(function($) {
  ...
  function _click(e) {
    ...
    const img_point = this.viewer.viewport.windowToImageCoordinates(
  ...
  function getRect(center, size, viewer, isPoint = false) {
    const max = viewer.viewport.imageToViewportCoordinates(
      viewer.world.getItemAt(0).source.dimensions
    );
    const width = size.x;
    const height = size.y;
  ...
  $.Patch = function(options) {
    ...
    if (
      ...
      Array.isArray(this.manager.selection)
    ) {
    ...
    if (this.manager && this.manager.selection) {
      ...
      sel.addEventListener(
        ...
        function(e) {
          ...
          input.value = color;
          element.style.borderColor = color;
          const idx = this.manager.selection.findIndex(
            elt => color == elt.color
          );
      ...
      if (hasColor && !this.isPoint)
        this.element.querySelector(".color").style.display = "none";
    }
    ...
    const tilIcon = this.element.querySelector(".til");
    const tilPanel = this.element.querySelector(".til_panel");
    ...
    if (
      ...
      this.manager.selection &&
      this.manager &&
      this.manager.selection[0].color &&
      this.isPoint
    )
      this.patchTrackers["color"].setTracking(false);
    ...
    if (options.data != "sROI") {
      this.patchTrackers["til"] = new $.MouseTracker({
      ...
      this.patchTrackers["til_range"] = new $.MouseTracker({
        element: tilIcon.querySelector("input[type=range]")
      });
    }
  ...
  $.Patch.prototype.setSizeText = function(e) {
    ...
    const rect = this.viewer.viewport.viewportToImageRectangle(
      this.overlay.getBounds(this.viewer.viewport)
    );
    this.element.querySelector(".info_block").textContent = `${Math.round(
      rect.width
  ...
  function moving(e) {
    ...
    if (this.parent) {
      const { x, y, width, height } = this.parent.overlay.getBounds(
        this.viewer
      );
  ...
  function resizing(e) {
    ...
    if (
      0 > img_point.x ||
      imgWidth < img_point.x ||
      0 > img_point.y ||
      imgHeight < img_point.y
    )
      return;
    const delta = this.viewer.viewport.deltaPointsFromPixels(e.delta, true);
    ...
    const sizeInScreen = this.viewer.viewport.deltaPixelsFromPoints(
  ...
  $.Patch.prototype.getTilDensity = function() {
    if (this.element.querySelector(".til input[type=checkbox]").checked) {
      return +this.element.querySelector(".til input[type=range]").value;
    } else {
      return "None";
    }
  };
  $.Patch.prototype.active = function() {
    if (this.viewer.pmanager.activePatch)
      this.viewer.pmanager.activePatch.deactive();
  ...
  $.Patch.prototype.destroy = function() {
    ...
    if (index != -1) this.viewer.pmanager.patches.splice(index, 1);
  };
  ...
  $.Patch.prototype.templateOptions = function() {
    ...
    if (this.manager && this.manager.selection) {
      options.data = this.element.querySelector(
        ".note_panel select option:checked"
      ).text;
   ...
   */
  $.Patch.prototype.getBounds = function(coordinates = "normalize") {
    // normalize or image
  ...
  $.Patch.prototype.getRect = function(coordinates = "normalize") {
    // normalize or image
  ...
  function createPatchElement(elt, rect, options) {
    ...
    // til density
    if (options.data != "sROI") {
      const til_ctrl = document.createElement("div");
      til_ctrl.classList.add("til");

      // check
      const _id = elt.id + "_til";
      const til_check = document.createElement("input");
      til_check.id = _id;
      til_check.type = "checkbox";
      ...
      // til label
      const til_scale = document.createElement("div");
      til_scale.classList.add("label");
      til_scale.textContent = 0;
      // til range
      const til_range = document.createElement("input");
      const rangeHandler = e => {
        til_scale.textContent = til_range.value;
      };
      til_range.addEventListener("input", rangeHandler);

      til_range.type = "range";
      til_range.value = 0;
      til_range.min = 0;
      til_range.max = 100;
      til_range.step = 1;
      ...
      til_ctrl.appendChild(til_range);
      til_ctrl.appendChild(til_scale);

      // elt.appendChild(til_range)
      elt.appendChild(til_ctrl);
      //elt.appendChild(til_panel);
    }
